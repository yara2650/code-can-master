...
class Calculation4Class extends StatefulWidget{
  State<Calculation4Class>createState()=> _pg();
}
class _pg extends State<Calculation4Class> {
  Reader obg=Reader();
  SqlDb sql= SqlDb();
  ...
  Widget _buildPortraitLayout(){
    return Column(
      children: [
        ...
        SizedBox(height:MediaQuery.of(context).size.height*.05,),
        Image.asset('images/orange.png',height:MediaQuery.of(context).size.height*.2,
          width:MediaQuery.of(context).size.width*.7 ,),
        SizedBox(height:MediaQuery.of(context).size.height*.05,),
        Column(
          children: [
            Row(
              //إذا قطعت البرتقالة بالنصف، كم عدد القطع التي ستحصل عليها
              mainAxisAlignment:MainAxisAlignment.center ,
              children: [
                Text("إذا قطعت البرتقالة بالنصف، كم",style: TextStyle(fontSize:MediaQuery.of(context).size.width*.09,color: Color(0xff543686) ),),
                Text("(2)",style: TextStyle(fontSize:MediaQuery.of(context).size.width*.09,color: Color(0xff543686)  )),
              ],
            ),
            Center(
              child: Text("عدد القطع التي ستحصل عليها"+"؟",style: TextStyle(fontSize:MediaQuery.of(context).size.width*.09,color: Color(0xff543686)  )),
            ),
            // Center(
            //   child: Text(" حتة"+"؟",style: TextStyle(fontSize:MediaQuery.of(context).size.width*.09,color: Color(0xff543686)  )),
            // ),
          ],
        ),
        GestureDetector(
            onTap: ()async{
              question=await sql.readData("SELECT question FROM 'questiondata' WHERE department='c' AND id=4 ");
        ...
        GestureDetector(
          onTap: ()async{
            List<Map> question=await sql.readData("SELECT grade , answer FROM 'questiondata' WHERE department='c' AND id=4 ");
            ...
            Navigator.of(context).pushNamed("ChoiceCalculation",
                arguments: {"nextpage":"Calculation5Class","grade":grade,"answer":answer});
          },
          child:Container(
            width: MediaQuery.of(context).size.width*.4,
  ...
  Widget _buildLandscapeLayout(){
    return Column(
      children: [
        ...
        SizedBox(height:MediaQuery.of(context).size.height*.05,),
        Image.asset('images/orange.png',height:MediaQuery.of(context).size.height*.25,
        ...
        Column(
          ...
          children: [
            Row(
              ...
              mainAxisAlignment:MainAxisAlignment.center ,
              children: [
                Text(" إذا قطعت البرتقالة بالنصف، كم عدد القطع التي ستحصل عليها"+"؟",style: TextStyle(fontSize:MediaQuery.of(context).size.width*.03,color: Color(0xff543686) ),),
                Text("(2)",style: TextStyle(fontSize:MediaQuery.of(context).size.width*.03,color: Color(0xff543686)  )),
              ],
            ),
